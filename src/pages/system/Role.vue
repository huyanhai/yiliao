<template>
  <div>
    ...
    <my-table :data-source="[1]" :pagination="pagination" @change="onPageChange">
      ...
      <a-table-column key="name" title="角色描述" data-index="lastName" />
      ...
      <a-table-column key="name" title="操作" data-index="lastName">
        <template #default="{ record }">
          <a-space>
            <a-button type="primary" size="small" @click="edit(record)">权限</a-button>
          </a-space>
        </template>
      </a-table-column>
    </my-table>
...
import MyDialog from './components/RuleDialog.vue';
